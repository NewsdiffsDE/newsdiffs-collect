...
SITES.forEach( SITE => {
    ...
    const teasers = JSON.parse(fs.readFileSync(`./../storage/${SITE.id}.json`), 'utf8');
    ...
    for ( index in teasers ) {
        ...
        const teaserHtml = renderTeaserDiff(teaser, SITE)
        pageHtml = pageHtml + teaserHtml    
    }
    pageHtml = pageHtml + '</ul>'
    const TEMPLATE_INDEX = fs.readFileSync('./template_diffie.html', {encoding: 'utf-8'})
    const RENDER_OBJECT = { pageHtml }
    const htmlIndex = eval_template(TEMPLATE_INDEX, RENDER_OBJECT)

    fs.writeFileSync(`./../www/diffie_${SITE.id}.html`, htmlIndex)
})
